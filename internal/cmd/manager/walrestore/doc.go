...
//
// Replica clusters are not affected: the designated primary of a replica
// cluster replicates from an external primary and runs in standby mode, so a
...
// The retry loop applies only when cluster.Status.TargetPrimary == podName,
// which is true in both dangerous scenarios and false for normal standby
// replicas:
//
//   - Post-promotion: CNPG sets TargetPrimary to the promoted pod before the
//     instance manager triggers promotion, so the cache is consistent by the
//     time WAL recovery begins.
//   - Bootstrap: setPrimaryInstance (cluster_create.go) sets TargetPrimary
//     before the restore job is created, well before the PostgreSQL pod starts
//     calling restore_command.
//
